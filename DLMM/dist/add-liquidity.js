...
const maxLiquidityDistributions = await Promise.all(maxPositionList.map(async (position) => {
    ...
    const startIndex = maxLiquidityDistribution.findIndex((item) => item.relativeBinId === range[0]) ?? 0;
    const endIndex = (maxLiquidityDistribution.findIndex((item) => item.relativeBinId === range[1]) ?? maxLiquidityDistribution.length - 1) + 1;
    const liquidityDistribution = maxLiquidityDistribution.slice(startIndex, endIndex);
    const binArray = binArrayList.find((item) => item.binArrayLowerIndex * 256 <= binLower &&
...
await Promise.all(maxLiquidityDistributions.map(async (maxLiquidityDistribution) => {
    ...
    const amountX = Number(new bigDecimal(Math.pow(10, PYUSD_TOKEN_DEVNET.decimals)).multiply(new bigDecimal(0.1)).getValue());
    const amountY = Number(new bigDecimal(Math.pow(10, WSOL_TOKEN_DEVNET.decimals)).multiply(new bigDecimal(0.1)).getValue());
