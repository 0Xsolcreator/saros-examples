...
const convertBalanceToWei = (strValue, iDecimal = 9) => {
    if (strValue === 0)
        return 0;
    try {
...
const maxLiquidityDistributions = await Promise.all(maxPositionList.map(async (position) => {
    ...
    const findStartIndex = maxLiquidityDistribution.findIndex((item) => item.relativeBinId === range[0]);
    const startIndex = findStartIndex === -1 ? 0 : findStartIndex;
    const findEndIndex = maxLiquidityDistribution.findIndex((item) => item.relativeBinId === range[1]);
    const endIndex = findEndIndex === -1 ? maxLiquidityDistribution.length : findEndIndex + 1;
