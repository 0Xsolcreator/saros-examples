...
import { Context, Data, Notification, ResourceId, SubscriberEvent } from './data-model';
 ...
 */
export type DispatchStrategy<T extends object> = BroadcastDispatchStrategy | UnicastDispatchStrategy<T> | MulticastDispatchStrategy<T>;
...
export interface UnicastDispatchStrategy<T extends object> extends BaseDispatchStrategy {
    dispatch: 'unicast';
    to: (ctx: Context<T>) => ResourceId;
}
export interface MulticastDispatchStrategy<T extends object> extends BaseDispatchStrategy {
    dispatch: 'multicast';
    to: (ctx: Context<T>) => ResourceId[];
}
...
export interface AddSinksStep<T extends object, R> {
    ...
    dialectSdk(adapter: (data: Data<R, T>) => DialectSdkNotification, dispatchStrategy: DispatchStrategy<T>): AddSinksStep<T, R>;
    ...
    solflare(adapter: (data: Data<R, T>) => SolflareNotification, dispatchStrategy: DispatchStrategy<T>): AddSinksStep<T, R>;
    custom<N extends Notification>(adapter: (data: Data<R, T>) => N, sink: NotificationSink<N>, dispatchStrategy: DispatchStrategy<T>): AddSinksStep<T, R>;
